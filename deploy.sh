#!/usr/bin/env bash
# deploy.sh — install or tear down the enterprise-platform on a local kind cluster.
#
# Usage:
...
#
# Reuses an existing kind cluster named enterprise-platform if present.
...
SEALED_SECRET_NAMES=(enterprise-platform-postgres enterprise-platform-keycloak-admin enterprise-platform-grafana-admin enterprise-platform-litellm-secrets enterprise-platform-s3-creds)
...
helm_release() { echo "enterprise-platform-$1"; }
    ...
    spec:
      ...
      containers:
        - name: bucket-init
          ...
          # S3 creds from the sealed enterprise-platform-s3-creds Secret (now that
          # seaweedfs S3 auth is on). Supplies AWS_ACCESS_KEY_ID,
          # AWS_SECRET_ACCESS_KEY, AWS_DEFAULT_REGION.
          envFrom:
            - secretRef:
                name: enterprise-platform-s3-creds
          command:
            - /bin/sh
            - -c
            - |
              set -eu
              ENDPOINT=http://enterprise-platform-seaweedfs-all-in-one:8333
...
bootstrap_litellm_db() {
  if kubectl -n "$NS" exec deploy/enterprise-platform-postgres -- \
    ...
    step "Creating litellm database in enterprise-platform-postgres"
    kubectl -n "$NS" exec deploy/enterprise-platform-postgres -- \
...
apply_grafana_dashboards() {
  ...
  for json in "$dash_dir"/*.json; do
    ...
    local cm="enterprise-platform-grafana-dashboard-${base}"
