...
Keycloak IdP for the `agentic-platform` namespace. Runs in **dev mode** (H2,
...
```bash
helm -n agentic-platform upgrade --install platform-keycloak deploy/helm/keycloak --wait
```
...
- Dev mode uses H2 — state is lost on pod restart (realm re-imports). For a durable
  setup, switch to `start` with `KC_DB=postgres` against `platform-postgres` and a
