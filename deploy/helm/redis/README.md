...
```bash
helm install enterprise-platform-redis deploy/helm/redis \
...
```bash
kubectl -n enterprise-platform port-forward svc/enterprise-platform-redis 6379:6379
...
```bash
helm uninstall enterprise-platform-redis -n enterprise-platform
kubectl delete pvc enterprise-platform-redis -n enterprise-platform
```
