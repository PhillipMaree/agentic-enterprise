...
key — into a normal `Secret` in the `enterprise-platform` namespace.
...
|---|---|---|---|
| `postgres.sealed.yaml` | `enterprise-platform-postgres` | `POSTGRES_USER/PASSWORD/DB` | postgres (`envFrom`), litellm (`db.secret`) |
| `keycloak.sealed.yaml` | `enterprise-platform-keycloak-admin` | `username`, `password` | keycloak (`secretKeyRef`) |
| `grafana.sealed.yaml` | `enterprise-platform-grafana-admin` | `admin-user`, `admin-password` | grafana (`admin.existingSecret`) |
| `litellm.sealed.yaml` | `enterprise-platform-litellm-secrets` | `masterkey` | litellm (`masterkeySecretName`) |
| `s3.sealed.yaml` | `enterprise-platform-s3-creds` | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION`, `S3_ENDPOINT_URL`, `seaweedfs_s3_config` | seaweedfs (`existingConfigSecret`), mlflow/tempo/loki (`envFrom`), bucket-bootstrap Job |
